...
async def get_orphan_models() -> list[OrphanModel]:
    ...
    for flow in installed_flows.values():
        for model in flow.models:
            for i in await get_possible_final_paths_for_model(installed_models, model):
    ...
    for flow in all_known_flows.values():
        for model in flow.models:
            model_save_paths = {str(i) for i in await get_possible_final_paths_for_model(installed_models, model)}
    ...
    for node_details in BASIC_NODE_LIST.values():
        ...
        for model_from_node in node_details["models"]:
            for i in await get_possible_final_paths_for_model(installed_models, model_from_node):
