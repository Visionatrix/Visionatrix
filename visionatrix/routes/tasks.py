...
async def get_task_progress(request: Request, task_id: int) -> TaskDetails:
    ...
    """
    r = await get_task(task_id, request.scope["user_info"].user_id, fetch_child=True)
    if r is None:
...
async def restart_task(
    ...
    """
    r = await get_task(task_id, request.scope["user_info"].user_id)
    if r is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Task `{task_id}` was not found.")
    if r["progress"] == 100.0:
    ...
    await task_restart_database(task_id)
...
async def delete_task(request: Request, task_id: int = Query(..., description="ID of the task to remove")):
    ...
    """
    r = await get_task(task_id, fetch_child=True)
    if r is None:
...
async def get_task_results(
    ...
    **Returns:**
    ...
    """
    task = await get_task(task_id, request.scope["user_info"].user_id)
    if task is None:
...
async def update_task_progress(
    ...
    if r["user_id"] != user_info.user_id and not user_info.is_admin:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Task `{task_id}` was not found.")
    update_success = await update_task_progress_database(
...
async def update_task_info(
    ...
    success = await update_task_info_database(task_id, update_fields)
    if not success:
